...
class wechatCallbackapiTest
{
    ...
    private function receiveText($object)
    {
       ...
       // $resultStr = $this->transmitText($object, $contentStr, $funcFlag);
		$resultStr = $this->myserver($object);
        return $resultStr;
    }
    ...
    private function updatetemplate($object)
    {
        ...
        $sqlstr =" select id, wxid,sendstatus,returnstatus,receivetime,sendtime "
            ." from t_wxsendtemplate where returnstatus is null and sendstatus ='ok'  "
                ." and TIMESTAMPDIFF(MINUTE,sendtime,now())<=1 and wxid ='".$wxid."' order by sendtime ";
       // error_log(date('Y-m-d H:i:s') . ":data =".$sqlstr."\r\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
        $product_array = get_mydata($sqlstr);
        foreach($product_array as $row) {
            $id =$row['id'];
            $sqlstr1 =" update t_wxsendtemplate set returnstatus ='".$status."',receivetime='"
                .$receivetime."' where id =".$id;
          //  error_log(date('Y-m-d H:i:s') . ":data =".$sqlstr1."\r\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
            sqlexec($sqlstr1);
            break;
    ...
    private function myserver($object)
    {
        $textTpl = "<xml>
		<ToUserName><![CDATA[%s]]></ToUserName>
		<FromUserName><![CDATA[%s]]></FromUserName>
	...
	public function sendregMsg()
	{
	    ...
	    $contentStr[] = array("Title" =>"木材人欢迎您加入  拥有木材人您将拥有即时便捷的木材信息",
	        "Description" =>"",
	        "PicUrl" =>"http://www.sxuav.com/mcr/wx/images/mcrzc1.jpg",
	        "Url" =>"http://www.sxuav.com/mcr/");
	    $contentStr[] = array("Title" =>"走进我们  了解我们  拥有我们",
	        "Description" =>"",
	        "PicUrl" =>"http://www.sxuav.com/mcr/wx/images/ljwm2.png",
	        "Url" =>"http://wqx.sxuav.com/v-S608C9NHR4");
	    $contentStr[] = array("Title" =>"木材人使用手册",
	        "Description" =>"",
	        "PicUrl" =>"http://www.sxuav.com/mcr/wx/images/sysm1.png",
