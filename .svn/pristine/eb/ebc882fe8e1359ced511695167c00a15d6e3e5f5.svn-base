...
<dl class="panel">
	...
	<?php
		...
		if(strlen($where)>0){
			...
			$i=0;
			do{
				$str = "select p.cdkey,p.goodtype,s.stuffName,k.kindName,l.lenname,p.diameterlen,t.timbername,p.wide,p.thinckness,k.kind_bj,case when p.goodtype=1 then w.position else dumpposition end as position,DATE_FORMAT(p.updatetime,'%m-%d %H:%i') as updatetime from t_product p left outer join t_kind k on p.kindid=k.kindid left outer join t_stuff s on p.stuffid=s.stuffid left outer join t_length l on p.productlen=l.lenid left outer join t_timber t on p.timber=t.timberid left outer join t_spotposition w on p.spotpositionid=w.spotpositionid where p.salestatusid<>4 ".$where." order by updatetime DESC,productid desc limit 5";
				$product_array = getdata($str);
				//error_log(date('Y-m-d H:i:s') .'求购:'.$_SESSION['userid'].' '.$str."\n",3,$_SERVER['DOCUMENT_ROOT']."/log/sqlrun.log");
				$i++;
				//if($i>5) break;
			...
			if(is_array($product_array)){
				foreach($product_array as $row){
					echo '<a class="borBom" href=\''.($row["goodtype"]==1?'Detail.php':'dumpDetail.php').'?cdkey='.$row["cdkey"].'\'><ul class=\'list table suc5 flex borBottom\'>';
...
<ul>
	<li class="flex table">
		<div class="marginTop"><a href="buyrelease.php?buyid=" class="btnFixed">继续发布</a></div>
		<div class="marginTop"><a href="mybuy.php" class="btnFixed">我的求购</a></div>
	</li>
</ul>
</body>
