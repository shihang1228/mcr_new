<?php
   ...
   // response.setcharacterEncoding("utf-8");
	...
	$contact = chageEmptyStr($de_json["contact"]);//货主
	$contactphone = chageEmptyStr($de_json["contactphone"]);//货主手机
	...
	//形成推荐条件
	$sqlstr = '';  //推荐查询条件
	if($kindid!=0) $sqlstr = 'p.kindid='.$kindid.' and ';
	if($stuffid!=0) $sqlstr = $sqlstr.' p.stuffid='.$stuffid.' and ';
	if($productlen!=0 && strlen($productlen)>0) $sqlstr .= ' p.productlen='.$productlen.' and ';
	if($diameterlen>0) $sqlstr .= ' p.diameterlen='.$diameterlen.' and ';
	if($timber!=0) $sqlstr .= ' p.timber='.$timber.' and ';
	if($thinckness>0) $sqlstr .= ' p.thinckness='.$thinckness.' and ';
	if($wide>0) $sqlstr .= ' p.wide='.$wide.' and ';
	if(strlen($sqlstr)>0){
    ...
    if($cdkey==""){
		$cdkey=$userphone.date("ymdHis").rand(1000,9999)."2";
		 $query ="insert into t_product(userid,kindid,stuffid,productlen,diameterlen,wide,thinckness,"
				...
				."dumpposition,updatetime,goodtype,cdkey,content,salestatusid,salechgedtime,contact,contactphone) values ("
				.$userid.",".$kindid.",".$stuffid.",'".$productlen."',".$diameterlen.",".$wide.",".$thinckness.","
				.$tolerance.",".$widerange.",".$thincknessrange.",".$refcapacity.",'".$timber."','"
				.$dry."','".$newold."','".$timbertype."','".$knob."','".$blue."','".$worm."','".$decay."','".$climb."','".$slash."','".$ring."','".$oil."','".$black."','"
				.$part."','".$device."','".$fromid."','".$portid."','".$publishtime."','"
				.$dumpposition."','".$updatetime."',2,'".$cdkey."','".$content."',2,now(),'".$contact."','".$contactphone."')";
			//error_log(date('Y-m-d H:i:s').'散货'.$query."\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
	}
	else{
		 ...
		 "',publishtime='".$publishtime."',dumpposition='".$dumpposition."',updatetime='".$updatetime."',goodtype=2,content='".$content."',contact='".$contact."',contactphone='".$contactphone."' where cdkey='".$cdkey."'";
	}
	//*********数据输入****
    $conn = db_connect();
	...
	$insertresult =$conn->query($query);
		...
		if($count_json >0) { //有图片
			...
			if (is_array($compic)) {
				foreach ($compic as $drow)
				{
					...
					if(!unlink(str_replace('.jpg','s.jpg',$drow["productpic"]))) $tdeleted = false;
				}
			...
			{
					$mediaid =$mypics[$i];
					$filename = date('YmdHis').rand(1000,9999).$userid;    //产生一个随机文件名
	 ...
	 if($insertresult && $picresult && $deleted && $delfromdb && $tdeleted){
		 $conn->query("commit");//提交事务
		 //echo "success";//插入成功
		 $rtn_array["result"]="success";
		 $rtn_array["searchvalue"]=$sqlstr;
		 echo json_encode($rtn_array);//插入成功
         return; 
		 ...
		 //echo "error";//插入失败
		 $rtn_array["result"]="error";
		 $rtn_array["searchvalue"]='';
		 echo json_encode($rtn_array);//插入成功
         return; 
	 }
