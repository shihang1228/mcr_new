			 ...
			 success:function(json){
				 if(typeof json == 'object'){
					 ...
					 if(json.length == 0){
						...
						$("#panel dd").empty().append("<p class='no_result'>没有查询到您所要的内容</p>");
					 }else{
					 ...
					 for(var i=1;i<json.length;i++){
							...
							if(neirou.productlen == 0){
								...
								$productlen = neirou.productlen + "米";
							}

							$item = $(
							"<a href='detail.php?cdkey="+neirou.cdkey+"'>"+
								"<ul class='flex table ltable5'>"+
								 	"<li>"+neirou.carnum+"</li><li>"+$productlen+"</li><li>"+neirou.stuffname+"</li>" + $bianhua +"<li>"+neirou.dumpposition+"</li>"+
								"</ul>"+
							"</a>"
							);
							 $(".no_result").empty();
	...
	if($pagecount > $num){
		 $.ajax({
		  ...
		  +"&timber="+timber,
			 ...
			 success:function(json){
				 if(typeof json == 'object'){
					 var neirou,$row,iheight,$item,$bianhua,$productlen;
