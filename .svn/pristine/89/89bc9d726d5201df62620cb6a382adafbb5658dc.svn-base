...
<dl class="panel dlcenter">
	<dt class="flex marginTop">
		...
		?>
			...
			<div id="collect" class="center1"><i class="iconfont">&#x3702;</i><span>关注</span></div>
		<?php
			}else{
		?>
			<div id="collect" class="center1"><i class="iconfont">&#x3702;</i><span>已关注</span></div>
		<?php
			}
		?>
	</dt>
	...
	$(function(){
		$("#mesboard").click(function(e){
			...
			$.ajax({
				...
				type : "POST",
				data : "ly=1&cdkey=<?php echo $cdkey; ?>&title=<?php echo $title; ?>&description=<?php echo $description; ?>",
...
<script>
	$(function(){
		...
		$("#collect").click(function(){
			...
			var collect_text = $(this).find("i").siblings("span").text();
			...
			if(collect_text == '关注'){
				$.ajax({
					...
					success : function(json){
						$("#collect").html('<i class="iconfont">&#x3702;</i><span>已关注</span>');
					}
