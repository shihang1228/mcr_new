...
class class_weixin
{
    ...
    */
    ...
    {
        set_time_limit(0);//限制最大的执行时间默认30S
        $curl = curl_init();
	...
	public function sendtemplatemsg($wxid,$cdkey,$username,$carnum,$arrivetime,$kindname,$arriveposition)
	{
		...
		$template = array('touser' => $wxid,
                 ...
                 'topcolor' => "#FF0000",
                 'data' => array('first'    => array('value' => urlencode($username."您好，您的货物已到站"),
                                                    'color' => "#173177"
                                                     ),
 ...
 public function sendServiceMsg($portname,$arriveposition,$arrivetime)
 {
     ...
     if ($dataresult)
     {
         while($row=$dataresult->fetch_row())
         {
             //  "picurl":"http://www.sxuav.com/mcr/wx/images/dhtz2.png",
             $wxid=$row[0];
             // error_log(date('Y-m-d H:i:s') . ":wxid =".$wxid."\r\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
             $data='{
                 "touser": "'.$wxid.'",
                 ...
                 "news": {  
                     ...
                     {
                          "title":"您好,您关注的口岸将有一列木材进站，请登录木材人查找车皮号、完善货物规格\n'
                          .'","description":"",'
                          .'"picurl":"",
                           "url":"http://www.sxuav.com/mcr/"
                     },  
                     ...
                     {
                          "title":"为了尊重您的工作和休息,如果您48小时内不点击最新到货,我们将不再冒然给您推送消息",
                          "description":"",
                           "picurl":"",
                           "url":"http://www.sxuav.com/mcr/"
                     }        
