	 ...
	 public  function carnumExist()
	  {
		...
		$sql="select * from t_product d,t_spotposition p where carnum='".$this->carnum
		."' and goodtype=1 and d.spotpositionid = p.spotpositionid ";
	 ...
	 public  function insertRecord()
	  {
        ...
        $query =" insert into t_product(carnum,userid,kindid,stuffid,productlen,diameterlen,wide,thinckness,"
			...
			."spotpositionid,updatetime,goodtype,cdkey,perfect,contact,contactphone,content,train,trainDate) values ('"
			...
			.$this->partid."','".$this->device."','".$this->fromid."','".$this->portid."','".$this->publishtime."',".$this->spotpositionid.",'"
			.$this->updatetime."',".$this->goodtype.",'".$this->cdkey."',".$this->perfect.",'".$this->contact."','"
			.$this->contactphone."','".$this->content."','".$this->train."','"
			.$this->trainDate."')";
			
			
            error_log(date('Y-m-d H:i:s') .'dump->insertRecord:'. $query."\r\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
	 ...
	 public  function operationRecord()
	  {
		...
		//	return $dumppositionidresult;
	        ...
	        $partresult=$this->runRecord("part",$this->partid,$row->part);//取材位置
			...
			$spotpositionidresult=$this->runRecord("spotpositionid",$this->spotpositionid,$row->ordervalue);//当前位置
		...
		//  return $resultscore;3
		  $result= $resultoperation && $kindidresult && $stuffidresult && $productlenresult && $refnumresult
			 ...
			 &&  $contactphoneresult && $contentresult && $spotpositionidresult && $resultscore
			 &&  $trainresult && $trainDateresult;
