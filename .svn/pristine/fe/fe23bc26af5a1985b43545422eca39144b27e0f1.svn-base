...
<section>
	<form action="" method="get">
		<ul class="flex selectWrapper">
			...
			<li><input type="button"  class="button sousuo" id="numberInput" onclick="start(1)" value="搜索"></li>
		</ul>
	</form>
</section>
...
<script type="text/javascript" src="../js/buymanage.js"></script>
<script>
	$(function(){
		$('body').on('click','.ul_list li .select_btn',function(){
		 ...
		 $('body').on('click','#del',function(){
			...
			if(window.confirm("确定要删除吗？")){
				 window.location = "buymanagedel_bgo.php?buyid="+companyid; //提交的url
			 }else{
				return;
			 }
	   });
	   $('body').on('click','#down',function(){
			...
			if(window.confirm("确定要下架吗？")){
				 window.location = "buymanageout_bgo.php?buyid="+companyid; //提交的url
			 }else{
				return;
			 }
	   });
