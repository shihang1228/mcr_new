...
function start(flag){	
	...
	var jareaselect=0,status=0,jstuffselect=0,productlen=0,productwide=0,thinckness=0,diameterlen=0,timber=0,jkindselect=0,carnum=0,startTime,endTime;;
	...
	if(document.getElementById("diameterlen").value.length == 0){
	 ...
	 $.ajax({
		 url:'myspot_bgo.php',
		 type:'POST',
		data:"carnum="+carnum+"&salestatusid="+status+"&kindselect="+jkindselect+"&startTime="+startTime+"&endTime="+endTime+"&stuffselect="+jstuffselect
		 ...
		 success:function(json){
			 if(typeof json == 'object'){
				 ...
				 if(json.length == 1){
					...
					for(var i=1;i<json.length;i++){
						...
						if(neirou.kindname == "原木"){
							if ((neirou.diameterlen == 0) ||(neirou.timber ==0)) {
								...
								$bianhua = "<span>" + neirou.diameterlen + "</span><span>" + neirou.timber + "</span>";
							}
						}else{
						...
						if(neirou.completestatus != "下架"){
							$xiajia = "<b class='select_btn'><img src='images/img_2.png' alt='' width='40' height='40'/></b>";
						}else{
							$xiajia = "";
						}
					...
					$("#ul_list").prepend("<li class='totalNum'>总条数："+json[0].total+"</li>")
				}
			}
		 }//success
	 });
