...
<?php
	...
	$chatinfo = getdata("select m.mb_id,m.mb_text as content,mb_text2 as description,cdkey,DATE_FORMAT(m.mb_date,'%Y-%m-%d %H:%i:%s') as mb_date,u.userid,ifnull(u.username,'访客') as username,u.userpic,case mb_ly when 1 then concat('detail.php?cdkey=',cdkey) when 2 then concat('dumpdetail.php?cdkey=',cdkey) when 3 then concat('buydetail.php?buyid=',cdkey) else '' end as lyaddr from t_messageboard m left outer join t_user u on m.userid=u.userid order by mb_date desc limit 100");
?>
<header class="header">
							...
							?>
										<div class='message clearfix' <?php echo $i!=0?'':'id="anchor0" name="anchor0"';?>>
											...
											<div class='wrap-text'> 
												...
												<div><?php echo $row['lyaddr']!=''?'<a class="atextcolor" href="'.$row['lyaddr'].'">'.$row['content'].$row['description'].'</a>':$row['content'];?></div> <!--文本框内容-->
											</div> 
											<div class='wrap-ri'>
												<div clsss='clearfix'>
