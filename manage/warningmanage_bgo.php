<?php
	...
	if(strlen($startTime)>0 && strlen($endTime)>0){
		$filtervalue =$filtervalue." and (TIMESTAMPDIFF(day,'".$startTime."',p.traindate)>=0 and TIMESTAMPDIFF(day,p.traindate,'".$endTime."')>=0)";
	}
	...
	//error_log(date('Y-m-d H:i:s').'报警查询:'.$filtervalue."\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
     ...
     ." ifnull( (select stuffname from t_stuff where stuffid = p.stuffid),'') as stuffname,p.traindate ";
		 
	 $query2= " from t_product p,t_port r,t_user u,v_statuspos v ,t_salestatus s"
		." where TIMESTAMPDIFF(day,p.traindate,now())>=3 and p.salestatusid<>4 and p.spotpositionid=3 and p.userid = u.userid and p.portid =r.portid  and goodtype=1"
   ...
   /*   $filterstr="";
	...
	//error_log(date('Y-m-d H:i:s').'报警查询:'.$query1.$query2."\n",3,$_SERVER['DOCUMENT_ROOT']."/log/run.log");
